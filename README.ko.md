...
Python **3.12**와 [`uv`](https://github.com/astral-sh/uv)가 요구된다. 두 기본 모델 모두 **게이트가 없기에**, Hugging Face 로그인이 필요하지 않다. 아래 명령은 실제 `drift` CLI이며(`scripts/install.sh`가 이를 제공한다).
...
이것이 전부다 — **레이어 범위도, IP도, 디바이스 플래그도 없다.** `drift run`은 모델의 레이어 수를 읽어, 발견한 노드들에 걸쳐 이를 분할하고 답을 스트리밍하며, 각 머신은 오직 자기 조각만 계산한다. (LAN에 zeroconf가 없는가? `drift run --nodes host:port,…`.) Gemma 4를 시도하려면 `config.yaml`에 `model_id: google/gemma-4-E2B-it`를 설정하면 된다 — 분할은 자동으로 다시 계산된다.

모델과 분할 지점을 바꾸는 법, 손수 구동하는 법, 비트 단위 패리티 게이트, 그리고 벤치마크는 모두 **운영 매뉴얼 → [docs/manual.ko.md](docs/manual.ko.md)** ([English](docs/manual.md) · [中文](docs/manual.zh.md) · [日本語](docs/manual.ja.md))에 있다.
