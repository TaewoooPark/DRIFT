...
<p align="center"><sub>뉴욕의 친구가 잠든 사이 노드를 켜 두고, 당신은 서울에 있다. DRIFT는 <b>하나의</b> 모델을 두 머신에 쪼갠다. 친구의 GPU가 앞쪽 레이어를, 당신의 GPU가 뒤쪽을 계산하고, 오직 hidden state만 바다를 건넌다. 그리하여 어느 한 대도 홀로 담지 못할 모델을 둘이 함께 돌리되, 그 답은 단일 머신과 비트 단위로 같다.</sub></p>
...
| 글자 | 의미 |
|---|---|
| **D**: Decentralized(탈중앙) | 데이터센터가 없다. 하이퍼스케일러가 아니라 이종의 개인 기기들이 모여 한 모델을 함께 돌린다. *정직하게 밝히자면, 지금은 오케스트레이터가 여전히 경로를 조율하고 embed/head를 쥐고 있어 단일 장애점이다. 노드끼리 직접 스트리밍하는 P2P 데이터 플레인과 특권을 내려놓은 head는 아래 로드맵에서 지금 만들고 있는 것이지, 아직 완성된 것은 아니다.* |
...
> **본 저장소의 범위.** 기술적 핵심, 곧 *Mac과 Windows 박스에 걸쳐 분할한 모델이 올바른 답을 내놓는가?* 는 완성됐고 비트 단위로 입증됐다. 그 위에서 **"For Tokens"** substrate에는 이제 작동하는 v1 씨앗들이 자란다. 신뢰 없는 스팟체크 검증, 어디서나 참여, 내결함성, 그리고 한 대엔 안 들어갈 모델 실행이다(아래 "분할 너머" 참조). 완전한 토큰 이코노미와 합의는 여전히 비전이다.
...
- **제어 플레인(control plane)**: 오케스트레이터가 노드를 순서대로 라우팅하며 리더 선출은 없다. 노드는 세 가지 방식으로 발견된다. 무설정 LAN 디스커버리(mDNS), 명시적 `--nodes host:port` 목록, 또는 NAT 뒤·Colab·클라우드 VM의 노드라면 `drift node --tunnel`이 여는 공개 `bore.pub` 터널이다.
...
// ping 응답  →  { "ok": true, "assigned", "name", "start_layer", "end_layer",
//                 "device", "torch", "transformers", "endian" }
```

`configure`는 fungible 노드에 레이어 범위를 배정하며(사용자가 범위를 손으로 적을 필요가 없다), ping의 `torch`/`transformers`/`endian`은 헤드가 레이어 배정 전에 버전이나 바이트 순서 불일치를 거부하게 한다.
...
**MPS ↔ CUDA (M4): 실측 완료.** 앞 절반을 Mac(Apple MPS), 뒤 절반을 Colab NVIDIA T4(CUDA)에서 돌린 결과, 분할 경로가 단일 머신 레퍼런스를 **정확히 재현했다. 3개 프롬프트에서 130/130 토큰 일치, 발산 없음.** 두 벤더의 fp16 커널 차이(및 torch 2.11 대 2.12 버전 차이)로 첫 스텝 logit 격차는 ~2×10⁻²까지 벌어졌으나(같은 device는 ~8×10⁻³) argmax를 뒤집기엔 부족했다. 더 큰 규모에서는 이 격차가 뒤쪽 토큰을 뒤집을 수 있는바, 그때를 위해 **완화된 게이트** `python -m drift.parity_test --prefix-match K`가 있다. 공개 터널을 통한 크로스머신 처리량은 ~2.7 tok/s(네트워크 바운드)였다. 토큰 1–2에서의 발산은 부동소수점 잡음이 아니라 명백한 **버그**인바 → 이분 탐색으로 넘어간다.
...
가장 무거운 단일 노드가 모델의 **42.4 %**만 짊어진다. 어느 한 대에도 2배로 큰 모델이 두 대에 걸쳐 돌아간다. 파이프라인 분할이 존재하는 이유다. **이 수치는 각 노드의 계산 책임 비율이 아니라 장치에 실제로 할당된 메모리다.** 각 노드는 자기 조각만 적재하므로(`init_empty_weights` + 선택적 safetensors 읽기) 전체 모델이 어느 한 대에도 상주하지 않으며, 슬라이스 적재가 단일 머신 적재와 비트 단위로 동일함은 parity 게이트가 증명한다.
...
| 가중치 : 토큰당 와이어 | **≈ 970,000 ×** |
| TPOT: 인프로세스(M2) → TCP(M3) | 40.7 → 43.1 ms/token |
| 프로토콜 오버헤드 | **+2.45 ms/token** (홉당 ~1.2 ms, TPOT의 ~6 %) |

**같은 디코드 루프**가 두 전송 계층 위에서 돈다. 따라서 M3 − M2 차이는 프레임워크 중립 프로토콜의 *순수* 비용, 곧 MPS와 CUDA를 협력하게 만드는 바로 그것이다. localhost에서는 홉당 ~1.2 ms의 작은 왕복 비용으로, ~41 ms/token의 연산에 압도된다. 이 규모에서는 측정 노이즈가 커서 실행마다 0을 중심으로 몇 ms씩 오간다(이전 실행에서는 소폭 음수로 측정되기도 했다). (실제 LAN은 RTT가 더해지며, 이는 DRIFT와 무관하다.)
...
- **왜 속도보다 정확성을 먼저 증명하는가?** *이종 분할 추론을 정확히* 해내는 것이 어려운 부분이라 그것이 먼저다. 그 위에 동시성이 얹힌다. 샤드 서버가 세션마다 스레드를 두어, 한 세션의 네트워크 왕복을 다음 세션의 연산과 겹친다. 생성 내부의 배칭과 추측 디코딩(speculative decoding)은 향후 과제다.
- **왜 모든 노드에 전체 모델을 두지 않는가?** 각 노드는 자기 조각만 적재한다. `init_empty_weights`로 meta 장치 위에 뼈대만 세운 뒤, 그 노드가 실제로 실행하는 텐서(자신의 디코더 레이어, 또는 헤드의 `embed`/`norm`/`lm_head`)만 safetensors에서 읽어 장치에 올린다. 가장 무거운 노드도 가중치의 42%만 실제 메모리에 담으며, 슬라이스 적재가 단일 머신 적재와 비트 단위로 동일함은 parity 게이트가 증명한다.
...
| **M4** | 머신 간: Mac MPS + NVIDIA CUDA | Mac + CUDA(Colab 가능) | ✅ **실측** — 100% 토큰 일치, ~2.7 tok/s |
| **M5** | 부스 디스플레이 + 인터랙티브 스트리밍 | + Windows | ⬜ |
| **M6** | 우아한 노드 종료 복구 | 완료 | ✅ 깔끔한 `NodeUnavailable` + 재연결 |

Mac 전용 트랙(M0–M3)은 **정확성 리스크의 100 %**를 지며 이미 완료되고 검토되었다. M4(크로스벤더)와 M6(kill-node)는 실측·작동하며, 남은 것은 M5의 부스 디스플레이뿐이다.
...
**인터넷 너머.** 다른 네트워크의 노드가 `drift node --tunnel`을 실행하면 공개 `bore.pub:PORT`가 출력되고, 헤드는 거기에 붙기만 하면 된다: `drift run --nodes bore.pub:PORT --prompt "hello world"`. VPN도 포트포워딩도 필요 없다.
...
drift/
  ...
  shard_server.py   # 동시 TCP 서버: ping / configure / reset / prefill / decode
  orchestrator.py   # embed + norm + lm_head + sampler; 주입 가능한 전송 계층; 디코드 루프
  run.py, node.py   # `drift run` 헤드 + `drift node` 워커 (자동 분할·디스커버리·--tunnel)
  discovery.py      # 무설정 LAN 디스커버리 (mDNS)
  tunnel.py         # 공개 bore.pub 터널: NAT 뒤 노드도 어디서나 참여
  verify.py         # 트러스트리스 스팟체크: 노드 검증, 부정직 출력 격리
  reference.py      # M1 단일 머신 오라클
  parity_test.py    # M2/M3 게이트 + 다중 프롬프트 --selftest
  bench.py, bench_m4.py   # 단일 머신 + 크로스머신(M4) 벤치
  common.py         # 설정 + 동일 토큰화 (오라클과 분할 경로가 공유)
config.yaml         # 모델, dtype, 포트, 샤드 테이블
docs/               # 공개 문서. benchmarks.md · manual.ko.md
```
...
**네트워크가 내 토큰을 보는가?** 냉정하게 짚자. `input_ids`는 정수 토큰 id이지만 이는 *가역* 인코딩이다. 공개된 토크나이저를 가진 누구든 이를 곧바로 당신의 텍스트로 되돌릴 수 있고, Per-Layer-Embedding 모델에서는 다운스트림 샤드가 실제로 이 값을 필요로 한다. 그러니 **지금은 노드 운영자가 당신의 프롬프트를 읽을 수 있다.** KV도 날것의 문자열도 와이어를 넘지 않지만, "네트워크가 내 텍스트를 볼 수 없다"는 말은 사실이 아니다. 와이어 암호화(키를 공유한 노드만 스트림을 읽을 수 있게 하는 것)는 아래 로드맵에 있다. 대신 지금도 할 수 있는 것은, 노드가 자기 연산에 대해 거짓말하지 않는지 확인하는 일이다. `python -m drift.verify`가 고정 챌린지로 노드를 검증해 정직 봉투를 벗어난 출력을 격리한다(정직 노드는 비트 단위로 일치하고, 조작 노드는 적발된다).
...
**처리량은?** 샤드 서버는 동시 세션을 스레드별로 처리하므로, 여러 생성이 네트워크 왕복을 겹친다. 단일 생성 내부의 배칭과 추측 디코딩은 향후 과제다.
...
기술적 핵심(정합하고 비트 단위로 검증된 이종 분할)은 끝났다. 그 위에서, *탈중앙* 네트워크가 필요로 하는 조각들에 이제 작동하는 v1 씨앗이 자란다:

| 능력 | 하는 일 | 실행 |
|---|---|---|
| **한 대엔 안 들어갈 모델 실행** | 각 노드가 **자기 레이어가 든 shard만** 받아 적재 — 디스크·VRAM 양쪽에서 자기 조각만 | 충분한 노드로 분할 |
| **어디서나 참여** | `drift node --tunnel`이 NAT 뒤·Colab·VM의 노드를 공개 `bore.pub:PORT`로 노출 (무계정·무토큰) | `drift node --tunnel` |
| **노드 드롭 감내** | 실행 중 드롭을 감지해 1회 재연결, 아니면 노드명을 담은 깔끔한 `NodeUnavailable`로 표면화 (raw 트레이스백 아님) | 디코드 중 노드 kill |
| **낯선 노드 신뢰** | 헤드가 고정 챌린지를 던져 M4로 실측한 정직 봉투를 벗어난 출력을 격리 (정직 → 비트 일치, 조작 → 적발) | `python -m drift.verify --nodes …` |

완성된 이코노미가 아니라 씨앗이다. 검증은 스팟체크(합의·암호학 아님), 터널은 릴레이(DHT 아님), kill-node는 감지하되 복제하지 않는다. 그러나 *한 대로 못 할 것을 함·아무 노드나 도달·드롭 감내·낯선 이 신뢰*는 더 이상 다이어그램만이 아니다.

---

## 로드맵

- **M4: 머신 간 (완료).** Mac(Apple MPS) + Colab NVIDIA T4(CUDA)에서 실측했다. 분할 경로가 단일 머신을 **정확히 재현**했고(3개 프롬프트 130/130 토큰), 공개 터널로 ~2.7 tok/s였으며, 버전·바이트 순서 점검이 torch 버전 차이를 잡아냈다. `scripts/colab_node.py --bore` + `python -m drift.bench_m4`로 재현된다.
- **M5: 부스 디스플레이.** 각 노드가 자신의 실시간 레이어 범위 + 디바이스를 드러내 보이며, 오케스트레이터는 토큰을 *"앞쪽 절반은 Apple GPU가 사고하고, 뒤쪽 절반은 NVIDIA가"*와 같이 스트리밍한다.
- **M6: 우아한 노드 종료 (완료).** 디코드 도중 떨어진 노드를 감지해 1회 재연결하고, 아니면 노드명을 담은 깔끔한 `NodeUnavailable`로 표면화한다. 매끄러운 페일오버는 아직 없다(복제가 필요하다).
