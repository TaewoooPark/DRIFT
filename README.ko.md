...
**MPS ↔ CUDA (M4).** 마침내 진짜 머신 간 단계에 이르러서는 두 GPU 벤더의 커널이 fp16을 서로 다르게 반올림하기에, greedy 디코딩이 *뒤쪽* 토큰에서 발산할 수 있다. 이는 이미 예상된 바이며 **완화된 게이트**로 처리된다. `python -m drift.parity_test --prefix-match K`는 앞쪽 K개 토큰의 일치를 요구하고 그 뒤의 발산은 허용한다. 다만 토큰 1–2에서의 발산은 부동소수점 잡음이 아니라 명백한 **버그**인바 → 이분 탐색으로 넘어간다.
...
- **M4: 머신 간.** LAN 상에서 하나의 모델을 Mac(MPS)과 Windows(CUDA)에 함께 얹는다. 완화된 패리티 게이트(`--prefix-match K`)가 예상되는 MPS↔CUDA 부동소수점 발산을 흡수하고, 노드는 레이어 배정 전 ping에서 버전과 바이트 순서를 교차 확인한다. 남은 것은 두 번째 물리 머신뿐이다.
