...
**MPS ↔ CUDA (M4).** 마침내 진짜 머신 간 단계에 이르러서는 두 GPU 벤더의 커널이 fp16을 서로 다르게 반올림하기에, greedy 디코딩이 *뒤쪽* 토큰에서 발산할 수 있다. 이는 이미 예상된 바이며 **완화된 게이트**로 처리된다. 이 게이트는 이제 하니스에 `python -m drift.parity_test --prefix-match K`로 구현돼 있다(앞쪽 K개 토큰은 일치해야 하고, 뒤쪽 발산은 허용된다). 다만 토큰 1–2에서의 발산은 부동소수점 잡음이 아니라 명백한 **버그**인바 → 이분 탐색으로 넘어간다.
...
- **M4: 머신 간.** LAN 상에서 하나의 모델을 Mac(MPS)과 Windows(CUDA)에 함께 얹는다. 도구는 이미 준비됐다. 완화된 패리티 게이트(`--prefix-match K`)와 노드 버전·바이트 순서 검증(레이어 배정 전 ping에서 확인)이 지금 동작하므로, 남은 것은 두 번째 물리 머신뿐이다.
