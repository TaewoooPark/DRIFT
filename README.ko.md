...
**네트워크가 내 토큰을 보는가?** 와이어를 넘는 것은 정수 `input_ids`와 부동소수점 `hidden_states`뿐이니, 텍스트도, KV도 넘지 않는다. 그리고 소유하지 않은 노드를 신뢰할 필요도 없다. `python -m drift.verify`가 고정 챌린지로 노드를 검증해 정직 봉투를 벗어난 출력을 격리한다(정직 노드는 비트 단위로 일치하고, 조작 노드는 적발된다).
