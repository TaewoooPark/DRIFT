...
| 文字 | 意味 |
|---|---|
| **D** — Decentralized（非中央集権） | データセンターがない。ハイパースケーラーではなく、異種混在の個人デバイスが集まって 1 つのモデルを一緒に動かす。*正直に述べると、今はオーケストレーターが依然としてルートを調整し embed/head を握っており、単一障害点である。ノード同士が（ヘッドを介さず）直接ストリームする P2P データプレーンと、特権を手放したヘッドは、下記ロードマップで今まさに構築中のものであり、まだ出荷されていない。* |
...
- **制御プレーン** — オーケストレーターはノードを順にルーティングし、リーダー選出はありません。ノードは 3 通りで見つかります — ゼロ設定の LAN ディスカバリ（mDNS）、明示的な `--nodes host:port` リスト、あるいは NAT 越し / Colab / クラウド VM のノードは `drift node --tunnel` が開く公開 `bore.pub` トンネルです。
...
// ping レスポンス  →  { "ok": true, "assigned", "name", "start_layer", "end_layer",
//                      "device", "torch", "transformers", "endian" }
```

`configure` は fungible なノードにレイヤー範囲を割り当てます（ユーザーが範囲を手書きする必要はありません）。ping の `torch` / `transformers` / `endian` は、ヘッドがレイヤーを割り当てる前にバージョンやバイトオーダーの不一致を拒否できるようにします。
...
- **なぜ速度より先に正しさを証明するのか？** *異種混在の分割推論を厳密にする* ことが難所なので、そこが先です。並行性はその上に乗ります。シャードサーバーはセッションごとにスレッドを持ち、あるセッションのネットワーク往復を次のセッションの計算と重ねます。生成内のバッチ処理と投機的デコーディングは今後の課題です。
...
Mac のみで完結するトラック（M0–M3）は **正しさに関するリスクの 100 %** を占め、完了かつレビュー済みです。M4（ベンダーまたぎ）と M6（kill-node）は実測済みで動作し、残るは M5 のブース表示だけです。
...
**インターネット越し** — 別ネットワークのノードが `drift node --tunnel` を実行すると公開 `bore.pub:PORT` が出力され、ヘッドはそこに向けるだけです: `drift run --nodes bore.pub:PORT --prompt "hello world"`。VPN もポートフォワーディングも不要です。
...
drift/
  ...
  shard_server.py   # 並行 TCP サーバー: ping / configure / reset / prefill / decode
  orchestrator.py   # embed + norm + lm_head + sampler; 差し替え可能なトランスポート; デコードループ
  run.py, node.py   # `drift run` ヘッド + `drift node` ワーカー（自動分割・ディスカバリ・--tunnel）
  discovery.py      # ゼロ設定の LAN ディスカバリ（mDNS）
  tunnel.py         # 公開 bore.pub トンネル — NAT 越しのノードもどこからでも参加
  verify.py         # トラストレスな抜き取り — ノードを問い、不正直な出力を検出
...
docs/               # 公開ドキュメント — benchmarks.md · manual.ja.md
```
...
**ネットワークは私のトークンを見るのか？** 冷静に捉えましょう。`input_ids` は整数のトークン id ですが、これは *可逆* なエンコーディングです — （公開されている）トークナイザーを持つ者なら誰でも、それをそのままあなたのテキストに戻せますし、Per-Layer-Embedding モデルでは下流のシャードが実際にこの値を必要とします。ですから **今はノード運営者があなたのプロンプトを読めます。** KV も生の文字列もワイヤを越えませんが、「ネットワークは私のテキストを見られない」というのは正しくありません。ワイヤ暗号化（鍵を共有したノードだけがそのストリームを読める）は下記ロードマップにあります。代わりに今すぐできるのは、ノードが自身の計算について嘘をついていないか確かめることです。`python -m drift.verify` が固定チャレンジでノードを問い、誠実な包絡から外れた出力を検出します — 誠実なノードはビット単位で一致し、改ざんされたノードは検出されます。
...
**スループットは？** シャードサーバーは並行セッションをスレッド単位で処理するので、複数の生成がネットワーク往復を重ねます。単一生成内のバッチ処理と投機的デコーディングは今後の課題です。
