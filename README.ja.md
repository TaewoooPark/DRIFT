...
**MPS ↔ CUDA（M4）。** 真にマシンをまたぐステップに至って初めて、2 つの GPU ベンダーのカーネルは fp16 を異なる形で丸めます。そのため greedy デコーディングは *後半の* トークンで分岐する可能性があります — これは想定内であり、**緩和されたゲート**で扱います。これはハーネスに `python -m drift.parity_test --prefix-match K` として実装されています（先頭 K 個のトークンは一致必須、それ以降の分岐は許容）。トークン 1〜2 での分岐は浮動小数点ノイズではなく **バグ** です → 二分探索へ。
...
- **M4 — マシン間。** LAN 越しに 1 つのモデルを Mac（MPS）+ Windows（CUDA）で実行。ツールは既に揃っています——緩和されたパリティゲート（`--prefix-match K`）とノードのバージョン / バイトオーダー検証（レイヤー割り当て前の ping で確認）が今動くので、残るは 2 台目の物理マシンだけです。
