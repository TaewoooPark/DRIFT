...
*方法論・制御変数・公正な競合比較プロトコルは **[docs/08-benchmark-plan.md](docs/08-benchmark-plan.md)**。すべての数字は `python -m drift.bench` で再現できる。*
...
最も重い単一ノードでもモデルの **42.4 %** しか担わない——どちらか 1 台には 2 倍大きすぎるモデルが 2 台にまたがって動く。パイプライン分割が存在する理由そのものだ。
...
> チェリーピックした勝利ではなく、絶対的で再現可能な数字だ。Exo / llama.cpp RPC との `tok/s` 直接対決には同一マシンへのインストールが要る。その公正なプロトコルは **[docs/08](docs/08-benchmark-plan.md)**。今日下せる比較の主張は、上の能力マトリクス**に加えて**単一マシンと*証明可能に*同一な分散出力だ。
