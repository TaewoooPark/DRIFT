...
**MPS ↔ CUDA（M4）。** 真にマシンをまたぐステップに至って初めて、2 つの GPU ベンダーのカーネルは fp16 を異なる形で丸めます。そのため greedy デコーディングは *後半の* トークンで分岐する可能性があります — これは想定内であり、**緩和されたゲート**で扱います。`python -m drift.parity_test --prefix-match K` は先頭 K 個のトークンの一致を要求し、それ以降の分岐を許容します。トークン 1〜2 での分岐は浮動小数点ノイズではなく **バグ** です → 二分探索へ。
...
- **M4 — マシン間。** LAN 越しに 1 つのモデルを Mac（MPS）+ Windows（CUDA）で実行。緩和されたパリティゲート（`--prefix-match K`）が想定される MPS↔CUDA の浮動小数点の分岐を吸収し、ノードはレイヤー割り当て前の ping でバージョン / バイトオーダーを相互確認します。残るは 2 台目の物理マシンだけです。
