...
flowchart LR
    ...
    GATE -->|いいえ| BISECT["🔎 fp32 max-abs-diff<br/>+ レイヤー二分探索"]
...
**MPS ↔ CUDA（M4）。** 真にマシンをまたぐステップに至って初めて、2 つの GPU ベンダーのカーネルは fp16 を異なる形で丸めます。そのため greedy デコーディングは *後半の* トークンで分岐する可能性があります — これは想定内であり、**緩和されたゲート**（序盤のトークンが一致 + 出力に一貫性がある）で扱います。トークン 1〜2 での分岐は浮動小数点ノイズではなく **バグ** です → 二分探索へ。
...
最も重い単一ノードでもモデルの **42.4 %** しか担わない——どちらか 1 台には 2 倍大きすぎるモデルが 2 台にまたがって動く。パイプライン分割が存在する理由そのものだ。**v0.10 以降、これらは各ノードの計算分担ではなく、デバイス上で実測されたメモリ割り当てです。**各ノードは自分のスライスだけを実体化するため（`init_empty_weights` + safetensors の選択的読み込み）、モデル全体がどの 1 台にも常駐せず、スライスロードが full ロードとビット単位で同一であることは parity ゲートが証明します。
...
| TPOT — プロセス内 (M2) → TCP (M3) | 43.3 → 42.6 ms/token |
| プロトコルオーバーヘッド | **ノイズの範囲内**（\|Δ\| < 1 ms/token、TPOT の約 1.6 %） |

**同一のデコードループ**が両トランスポート上で走るので、M3 − M2 の差はフレームワーク中立プロトコルの*純粋な*コスト——MPS と CUDA を協調させるまさにそれ——だ。localhost では 0 と区別できない。3 KB の往復は計算に圧倒される。（実際の LAN は RTT が上乗せされるが、それは DRIFT とは無関係だ。）
...
- **なぜ各ノードにモデル全体を置かないのか？** v1 は単純さのためそうしていました。**v0.10** からは各ノードが自分のスライスだけをロードします：`init_empty_weights` で meta デバイス上に骨組みを作り、そのノードが実際に実行するテンソル（自分のデコーダ層、またはヘッドの `embed`/`norm`/`lm_head`）だけを safetensors から読み、デバイスに載せます。最も重いノードでも重みの **42 %** しか実メモリに持たず、スライスロードが full ロードとビット単位で同一であることは parity ゲートが証明します。
