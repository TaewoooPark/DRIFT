...
最重的单节点只承担模型的 **42.4 %**——一个对任一台机器都大 2 倍的模型，可以跨两台一起跑。这正是流水线切分存在的理由。**自 v0.10 起，这些是设备上实测的分配量，而不仅是各节点的计算份额：**每个节点只实体化自己那一片（`init_empty_weights` + 按需读取 safetensors），因此完整模型永远不会驻留在任何一台机器上，而 parity 门禁证明切片加载与完整加载逐位相同。
...
- **为什么不在每个节点都保留完整模型？** v1 为了简单这样做；自 **v0.10** 起，每个节点只加载自己那一片：`init_empty_weights` 在 meta 设备上搭好骨架，然后只从 safetensors 读取该节点真正运行的张量（它的解码层，或头节点的 `embed`/`norm`/`lm_head`）并放到设备上。最重的节点在实际内存中也只占权重的 **42 %**，而 parity 门禁证明切片加载与完整加载逐位相同。
