...
flowchart LR
    ...
    GATE -->|yes| PASS["✅ 一致 —— 推进"]
    GATE -->|no| BISECT["🔎 fp32 最大绝对差<br/>+ 逐层二分定位<br/>(docs/05)"]
...
**MPS ↔ CUDA（M4）。** 只有到了真正的跨机步骤，两家 GPU 厂商的 kernel 才会对 fp16 做出不同的舍入，于是贪心解码可能在*较靠后*的 token 上发散——这是预期之内的，由一道**放宽的门禁**处理（前若干 token 吻合 + 输出连贯）。而在第 1–2 个 token 就发散则是 **bug**，不是浮点噪声 → 二分定位。专门针对这一情形的手册是 **[docs/05-parity-debugging-playbook.md](docs/05-parity-debugging-playbook.md)**。

本实现经过**连续两轮全部 OK** 的评审——涵盖正确性，以及与 Exo/Petals/llama.cpp 的设计对比——并从 Exo 自己的源码验证了那条关于 Exo 耦合的论断。记录在册：**[docs/07-review-log.md](docs/07-review-log.md)**。

---

## 基准测试

*方法论、控制变量与公平的竞品对比协议见 **[docs/08-benchmark-plan.md](docs/08-benchmark-plan.md)**。所有数字用 `python -m drift.bench` 复现。*
...
最重的单节点只承担模型的 **42.4 %**——一个对任一台机器都大 2 倍的模型，可以跨两台一起跑。这正是流水线切分存在的理由。
...
| TPOT——进程内 (M2) → TCP (M3) | 43.3 → 42.6 ms/token |
| 协议开销 | **在噪声以内**（\|Δ\| < 1 ms/token，约 TPOT 的 1.6 %） |

**同一个解码循环**跑在两种传输之上，所以 M3 − M2 的差值就是框架中立协议的*纯*成本——让 MPS 与 CUDA 协作的那个东西。在 localhost 上它与 0 无法区分：3 KB 往返被算力压得微不足道。（真实 LAN 会叠加 RTT，与 DRIFT 无关。）

> 这是绝对、可复现的数字，不是精挑细选的胜利。与 Exo / llama.cpp RPC 的 `tok/s` 正面较量需要把它们装在同一台机器上；公平的协议见 **[docs/08](docs/08-benchmark-plan.md)**。今天能下的对比结论是上面的能力矩阵**加上**一份*可证明*与单机相同的分布式输出。
...
有意思的决定，是 DRIFT 拒绝掉的那些。每一条都是规格说明里的硬约束。
...
- **为什么（v1 里）在每个节点都加载完整模型？** 为了简单和正确。省内存的路径（`init_empty_weights` + 按分片的 safetensors）是一个已知的后续项；v1 加载整个模型却只用其中一片，这样显然正确，而且在笔记本上放得下一个 1.5–4 B 的模型。
...
docs/               # 01 实现计划 · 02 工作流 · 03 目标 · 04 技能/MCP · 05 一致性排查手册 · 06 环境搭建 · 07 评审日志
```

**审阅者的重点清单：** `engine_torch.py`（KV 重索引 + 自省）、`protocol.py`（冻结的线缆）、`orchestrator.py`（可注入传输层 + 解码循环），以及 `docs/05`（一致性是如何调试的）。
...
- **M4 —— 跨机。** 在局域网上让 Mac（MPS）+ Windows（CUDA）共同运行一个模型；针对预期中的 MPS↔CUDA 浮点分歧采用放宽的一致性门禁；在两个节点上做版本锁定校验。
