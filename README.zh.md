...
| 字母 | 含义 |
|---|---|
| **D** —— Decentralized（去中心化） | 没有数据中心——是异构的个人设备，而非超大规模云厂商，一起运行这一个模型。*据实相告：如今仍由一个编排器协调整条路由并掌握 embed/head，因此它是单点故障。让节点彼此直接串流（而非经由头节点）的 P2P 数据平面，以及一个去特权化的头节点，是下方路线图正在建设的内容——尚未交付。* |
...
> **本仓库的范围。** 技术核心——*一个跨 Mac 与 Windows 切分的模型能否给出正确答案？*——已完成并逐位验证。在其之上，**"For Tokens"** 的底座如今长出了可用的 v1 幼苗：无信任抽查验证、随处加入、容错，以及运行单机放不下的模型（见下文"切分之外"）。完整的 token 经济与共识仍是愿景。
...
- **控制平面**——编排器按顺序路由到各节点，没有 leader 选举。节点有三种发现方式：零配置局域网发现（mDNS）、显式 `--nodes host:port` 列表，或对 NAT 后 / Colab / 云 VM 上的节点，由 `drift node --tunnel` 开启的公共 `bore.pub` 隧道。
...
// ping 响应  →  { "ok": true, "assigned", "name", "start_layer", "end_layer",
//                 "device", "torch", "transformers", "endian" }
```

`configure` 为可替换（fungible）节点分配层范围（用户无需手写范围）；ping 的 `torch` / `transformers` / `endian` 让头节点在分配层之前拒绝版本或字节序不匹配。
...
**MPS ↔ CUDA（M4）——已实测。** 前半在 Mac（Apple MPS）、后半在 Colab NVIDIA T4（CUDA）上运行，切分路径**逐字复现了单机参考：3 个提示词 130/130 token 全部一致，无发散**——尽管两家厂商的 fp16 kernel（以及 torch 2.11 与 2.12 的版本差异）把首步 logit 差距拉大到 ~2×10⁻²（单设备为 ~8×10⁻³），却不足以翻转 argmax。在更大规模上该差距可能翻转靠后的 token，为此才有**放宽的门禁** `python -m drift.parity_test --prefix-match K`。经由公共隧道的跨机吞吐约为 2.7 tok/s（受网络限制）。而在第 1–2 个 token 就发散则是 **bug**，不是浮点噪声 → 二分定位。
...
- **为什么先证明正确性再谈速度？** 把异构切分推理做到精确才是难点，所以先做。并发在其之上：分片服务器为每个会话开一个线程，让一个会话的网络往返与下一个会话的计算重叠。生成内的批处理与推测解码（speculative decoding）是未来工作。
...
| **M4** | 跨机——Mac MPS + NVIDIA CUDA | Mac + CUDA（Colab 即可） | ✅ **已实测**——100% token 一致，~2.7 tok/s |
| **M5** | 展台展示 + 交互式流式输出 | + Windows | ⬜ |
| **M6** | 优雅的 kill-node 恢复 | 完成 | ✅ 干净的 `NodeUnavailable` + 重连 |

仅 Mac 的这条线（M0–M3）承担**100% 的正确性风险**，已完成并已评审；M4（跨厂商）与 M6（kill-node）已实测且可用，只剩 M5 的展台显示尚未构建。
...
**跨越互联网** —— 另一网络上的节点运行 `drift node --tunnel`，它会打印一个公共 `bore.pub:PORT`，头节点只需指向它：`drift run --nodes bore.pub:PORT --prompt "hello world"`。无需 VPN，无需端口转发。
...
drift/
  ...
  shard_server.py   # 并发 TCP 服务器：ping / configure / reset / prefill / decode
  orchestrator.py   # embed + norm + lm_head + sampler；可注入传输层；解码循环
  run.py、node.py   # `drift run` 头 + `drift node` 工作节点（自动切分、发现、--tunnel）
  discovery.py      # 零配置局域网发现（mDNS）
  tunnel.py         # 公共 bore.pub 隧道 —— NAT 后节点也能随处加入
  verify.py         # 无信任抽查 —— 盘问节点，标记不诚实输出
...
docs/               # 公开文档 — benchmarks.md · manual.zh.md
```
...
**网络能看到我的 token 吗？** 请清醒看待：`input_ids` 是整数 token id，但那是一种*可逆*编码——任何持有（公开）分词器的人都能把它直接还原成你的文本，而且 Per-Layer-Embedding 模型的下游分片确实需要它。所以**如今节点运营者可以读到你的提示词。** 没有 KV、没有原始字符串跨越线缆，但"网络看不到我的文本"这句话是不成立的。线缆加密（只有与你共享密钥的节点才能读取该流）在下方路线图上。你现在就能做的，是核查一个节点没有在其算力上撒谎：`python -m drift.verify` 用固定挑战盘问节点，标记落在诚实包络之外的输出——诚实节点逐位一致，篡改节点被抓出。
...
**吞吐量怎么办？** 分片服务器按线程处理并发会话，因此多个生成重叠网络往返。单次生成内的批处理与推测解码是未来工作。
...
技术核心（正确、逐位验证的异构切分）已完成。在其之上，一个*去中心化*网络所需的部件如今长出了可用的 v1 幼苗：

| 能力 | 作用 | 试试 |
|---|---|---|
| **运行单机放不下的模型** | 每个节点**只下载并加载持有自身层的分片**——磁盘与 VRAM 上都只有自己那一片 | 切分到足够多的节点 |
| **随处加入** | `drift node --tunnel` 把 NAT 后 / Colab / VM 上的节点暴露为公共 `bore.pub:PORT`（无账号、无令牌） | `drift node --tunnel` |
| **容忍节点掉线** | 运行中掉线被检测并重连一次，否则以干净的 `NodeUnavailable` 表面化（而非原始堆栈） | decode 中途 kill 一个节点 |
| **信任你不拥有的节点** | 头节点用固定挑战盘问节点，标记落在 M4 实测诚实包络之外的输出（诚实 → 逐位一致；篡改 → 抓出） | `python -m drift.verify --nodes …` |

是幼苗，不是成型的经济：验证是抽查（非共识/密码学），隧道是中继（非 DHT），kill-node 只检测不复制。但*做单机做不到的 · 到达任意节点 · 容忍掉线 · 信任陌生者*已不再只是一张图。

---

## 路线图

- **M4 —— 跨机（已完成）。** 在 Mac（Apple MPS）+ Colab NVIDIA T4（CUDA）上实测：切分路径**逐字复现单机**（3 个提示词 130/130 token），经公共隧道约 2.7 tok/s，版本 / 字节序检查抓到了 torch 版本差异。用 `scripts/colab_node.py --bore` + `python -m drift.bench_m4` 可复现。
- **M5 —— 展台展示。** 每个节点实时显示自己的层范围 + device；编排器以*"前一半由 Apple GPU 思考，后一半由 NVIDIA 思考"*的方式流式输出 token。
- **M6 —— 优雅的 kill-node（已完成）。** decode 中途掉线的节点被检测并重连一次，否则以带节点名的干净 `NodeUnavailable` 表面化。尚无无缝故障转移（需要复制副本）。
