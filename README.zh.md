...
**MPS ↔ CUDA（M4）。** 只有到了真正的跨机步骤，两家 GPU 厂商的 kernel 才会对 fp16 做出不同的舍入，于是贪心解码可能在*较靠后*的 token 上发散——这是预期之内的，由一道**放宽的门禁**处理（前若干 token 吻合 + 输出连贯）。而在第 1–2 个 token 就发散则是 **bug**，不是浮点噪声 → 二分定位。
...
需要 Python **3.12** 和 [`uv`](https://github.com/astral-sh/uv)。两个默认模型都是**无需授权（ungated）**的——不需要 Hugging Face 登录。下面的命令就是真实的 `drift` CLI（由 `scripts/install.sh` 提供）。
...
这就是全部——**没有层范围，没有 IP，没有 device 标志。** `drift run` 读取模型的层数，把它切分到自己找到的各个节点上，再把答案流式返回；每台机器只计算自己那一片。（局域网上没有 zeroconf？用 `drift run --nodes host:port,…`。）要试 Gemma 4，在 `config.yaml` 里设好 `model_id: google/gemma-4-E2B-it`——切分会自动重新计算。

更换模型与切分点、手动驱动、逐位一致性门禁，以及基准测试，全都在**操作手册 → [docs/manual.zh.md](docs/manual.zh.md)** ([English](docs/manual.md) · [한국어](docs/manual.ko.md) · [日本語](docs/manual.ja.md)) 里。
