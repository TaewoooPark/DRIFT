...
**MPS ↔ CUDA（M4）。** 只有到了真正的跨机步骤，两家 GPU 厂商的 kernel 才会对 fp16 做出不同的舍入，于是贪心解码可能在*较靠后*的 token 上发散——这是预期之内的，由一道**放宽的门禁**处理，现已作为 `python -m drift.parity_test --prefix-match K` 落地在工具链中（前 K 个 token 必须吻合，之后的发散允许）。而在第 1–2 个 token 就发散则是 **bug**，不是浮点噪声 → 二分定位。
...
- **M4 —— 跨机。** 在局域网上让 Mac（MPS）+ Windows（CUDA）共同运行一个模型。工具已就绪——放宽的一致性门禁（`--prefix-match K`）与节点版本 / 字节序校验（在分配层之前于 ping 时检查）现已可用，因此只差第二台物理机器。
