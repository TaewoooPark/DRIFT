...
**1 台** のマシン（localhost）上で 2 つのシャードを立ち上げ、次に TCP 越しに実際の生成を行います。
...
最後のコマンドこそが製品そのものです。オーケストレーターはプロンプトを埋め込み、hidden
state をシャード A、続いてシャード B へとルーティングし、デコードした答えを表示します — 各シャードは
自分自身のレイヤーだけを実行します。以下はすべて、その実行の挙動を変える方法についてです。
