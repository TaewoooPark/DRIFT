...
한 번만 설치하면 된다 — `bash scripts/install.sh`(macOS/Linux) 또는 `powershell -File scripts\install.ps1`
(Windows) — 그다음 `drift doctor`로 상태를 점검하라. 실행하려면:
...
**레이어 범위도, IP도, 디바이스 플래그도 없다.** `drift run`은 모델의 레이어 수를 읽어, 발견한
노드들에 걸쳐 이를 분할하고, 각 노드는 오직 자기 조각만 계산한다. 아래 내용은 모두 그 실행이 하는 일을
바꾸는 방법이며 — §5–§6에서는 샤드를 손수 구동하는 법도 함께 다룬다.
...
**쉬운 방법.** 각 워커에서 `drift node`를 실행하면(디바이스를 자동 감지하고 스스로를 알린다), 헤드에서는
`drift run`을 실행한다(LAN 상에서 워커를 자동 발견하고, 모델을 분할하고, 스트리밍한다). IP도 범위도
없다. LAN이 mDNS를 막는다면 워커를 명시적으로 나열하라:
`drift run --nodes 192.168.0.22:PORT,192.168.0.11:PORT`. 이 절의 나머지는 **손수 하는** 방법으로 —
정확한 포트/범위를 고정하거나 완전한 수동 제어를 원할 때 유용하다.
...
| 명령 | 하는 일 |
|---|---|
| `drift doctor` | 사전 점검: Python/torch/디바이스, 의존성, `config.yaml` 타일링, 포트 도달성, 방화벽 힌트 |
| `drift up N` | localhost: N개 노드 스폰, 자동 분할, 채팅(또는 원샷이면 `--prompt`) |
| `drift node` | 이 머신을 워커로 실행: 자동 디바이스, LAN에 알림, 헤드를 기다림 |
| `drift run` | 헤드: 노드 발견(또는 `--nodes host:port,…`), 자동 분할, 구성, 스트리밍/채팅 |

`drift up`, `node`, `run`은 `--max-new-tokens`를 받고, `run`은 추가로 `--model`과 `--nodes`를
받는다. 이들은 아래의 하위 수준 모듈을 감싼 것이다 — 패리티 게이트와 벤치마크에는 모듈을 직접 사용하라.
